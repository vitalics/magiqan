...
import type { ClassResult, ClassTest, FileResult, FileTest, Hook, RunnerLike, Test, TestResult } from '@magiqan/types';
...
export class JSONReporter extends Reporter {
  ...
  onFileResult(fileResult: FileResult) {
    this._results.push(fileResult);
  }
  onRunnerInit(_runner: RunnerLike, cwd: string): void {
    this._resolvedPath = path.resolve(cwd, this.file);
  }

  async generate(): Promise<void> {
    return writeFileAsync(this._resolvedPath, JSON.stringify(this._results, null, 2));
  }
